...
from .models import Problem, ProblemRuleType, ProblemTag, ProblemIOMode, ProblemClassify, ProblemClassifyTwo
...
class CreateOrEditProblemSerializer(serializers.Serializer):
    ...
    classify = serializers.ListField(child=serializers.CharField(max_length=32), allow_empty=False)
    # classify_two = serializers.ListField(child=serializers.CharField(max_length=32), allow_empty=False)
...
class BaseProblemSerializer(serializers.ModelSerializer):
    ...
    classify = serializers.SlugRelatedField(many=True, slug_field="name", read_only=True)
...
# 分类序列
class CreateClassifySerializer(serializers.Serializer):
...
class EditClassifySerializer(serializers.Serializer):
    id = serializers.IntegerField()
    name = serializers.CharField(max_length=64)


# 子类序列
class CreateSubclassSerializer(serializers.Serializer):
    # parent = ClassifySerializer(many=True)
    parent = serializers.CharField(source='parent.id')
    name = serializers.CharField(max_length=64)
...
class SubclassSerializer(serializers.ModelSerializer):
    ...
    parent = serializers.CharField(source='parent.id')
    class Meta:
        model = ProblemClassifyTwo
        fields = "__all__"


class EditSubclassSerializer(serializers.Serializer):
    id = serializers.IntegerField()
    # parent = ClassifySerializer(many=True)
    parent = serializers.CharField(source='parent.id')
    name = serializers.CharField(max_length=64)