...
from ..models import Problem, ProblemRuleType, ProblemTag, ProblemClassify, ProblemClassifyTwo
...
from problem.serializers import (CreateClassifySerializer, ClassifySerializer, EditClassifySerializer,
                                 CreateSubclassSerializer, SubclassSerializer, EditSubclassSerializer)
...
class ProblemAPI(ProblemBase):
    ...
    def post(self, request):
        ...
        for item in classifys:
            try:
                classify = ProblemClassifyTwo.objects.get(name=item)
            except ProblemClassifyTwo.DoesNotExist:
                # classify = ProblemClassifyTwo.objects.create(name=item)
                return self.error("ProblemClassifyTwo does not exist")
            problem.classify.add(classify)
...
# 大类api 集合
class ClassifyAdminAPI(APIView):
    @validate_serializer(CreateClassifySerializer)
    ...
    def post(self, request):
        ...
        """
        data = request.data
        classify = ProblemClassify.objects.create(name=data["name"])
        return self.success(ClassifySerializer(classify).data)

    @validate_serializer(EditClassifySerializer)
    @super_admin_required
    def put(self, request):
        """
        edit announcement
        ...
        data = request.data
        try:
            classify = ProblemClassify.objects.get(id=data.pop("id"))
        except ProblemClassify.DoesNotExist:
            return self.error("ProblemClassify does not exist")
    ...
    def get(self, request):
        ...
        if classify_id:
            try:
                classify = ProblemClassify.objects.get(id=classify_id)
                return self.success(ClassifySerializer(classify).data)
            except ProblemClassify.DoesNotExist:
                return self.error("ProblemClassify does not exist")
        classify = ProblemClassify.objects.values()
        return self.success(self.paginate_data(request, classify, ClassifySerializer))

    @super_admin_required
    def delete(self, request):
        if request.GET.get("id"):
            ProblemClassify.objects.filter(id=request.GET["id"]).delete()
        return self.success()

# 子类api 集合
class SubclassAdminAPI(APIView):
    @validate_serializer(CreateSubclassSerializer)
    @super_admin_required
    def post(self, request):
        """
        publish announcement
        """
        data = request.data
        parent = ProblemClassify.objects.get(id=data["parent"])
        subclass = ProblemClassifyTwo.objects.create(parent=parent, name=data["name"])
        return self.success(SubclassSerializer(subclass).data)

    @validate_serializer(EditSubclassSerializer)
    @super_admin_required
    def put(self, request):
        """
        edit announcement
        """
        data = request.data
        # raise Exception('121212', data['parent'])
        try:
            subclass = ProblemClassifyTwo.objects.get(id=data.pop('parent'))
        except ProblemClassifyTwo.DoesNotExist:
            return self.error("ProblemClassifyTwo does not exist")
        ...
        return self.success(ClassifySerializer(subclass).data)
    ...
    def get(self, request):
        ...
        get announcement list / get one announcement
        """
        subclass_id = request.GET.get("id")
        if subclass_id:
            try:
                subclass = ProblemClassifyTwo.objects.filter(parent=subclass_id)
                return self.success(self.paginate_data(request, subclass, SubclassSerializer))
            except ProblemClassifyTwo.DoesNotExist:
                return self.error("ProblemClassifyTwo does not exist")
        subclass = ProblemClassifyTwo.objects.all()
        return self.success(self.paginate_data(request, subclass, SubclassSerializer))

    @super_admin_required
    def delete(self, request):
        if request.GET.get("id"):
            ProblemClassifyTwo.objects.filter(id=request.GET["id"]).delete()
        return self.success()
